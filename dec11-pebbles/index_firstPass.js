...
function* blink({ stones: originalStones = [] }, config) {
  ...
  for (let blink = 1; blink <= firstPass; blink++) {
    stones = stones.flatMap(change)
    if (showIntermediate) console.log({ msg: `1st pass -- Blink ${blink}`, stones: stones.length })
  }
  for (const stone of [...stones]) {
    let stones = [stone]
    for (let blink = firstPass+1; blink <= blinkCount; blink++) {
      stones = stones.flatMap(change)
    }
    count += stones.length
    if (showIntermediate) console.log({ msg: `Blink ${blinkCount}`, original: stone, stones: stones.length, count })
  }
  yield count
}
...
export default function* pickPart(input, config) {
  ...
  if (config.part === 1) {
    ...
    yield `After ${config.blinkCount} blinks, there are ${result} stones.`
  } else {
